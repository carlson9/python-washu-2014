...
data = pd.read_csv("https://raw.githubusercontent.com/carlson9/python-washu-2014/master/hw5/mathofpolitics.csv") #csv from hw5
...
class Blog(Base): #table for blog posts
    ...
    def __init__(self, url, is_post, publish_date, author, post_title, comment_count):
        ...
        if isinstance(publish_date, float): #means NaN and not a post
            ...
            self.post_title = unicode(str(post_title), encoding='utf-8') #had to convert scraped data
...
class Users(Base): #table to store user info - num of followers and num of tweets (activity measure)
    ...
    def __repr__(self):
        return 'name: %s\nNumber of followers: %s\nNumber of Tweets' %(self.name, self.number_followers, self.number_tweets)
...
userlist = []
i=0
while i<len(people): # Code below loops through all users who follow Matt, and continues to update who stored as the "most_followed" as loop runs.
    try:
        user = api.get_user(followers[i])
        userlist[i] = Users(str(user.name), user.followers_count, user.statuses_count)
        i+=1
